...
class OwnerInfoExtractor:
    ...
    # 고해상도 설정
    HIGH_DPI = 300  # 300 DPI (기존 150-200 → 300)
    MAX_IMAGE_PX = 2000  # 더 큰 이미지 허용
    ...
    def _extract_high_quality_images(
        ...
        if page_numbers:
            ...
            # ★★★ 모든 페이지 분석 (주택매도신청서 위치 모름) ★★★
            pages_to_extract = list(range(len(doc)))
            print(f"    [전체 페이지 분석] 총 {len(doc)}페이지")
