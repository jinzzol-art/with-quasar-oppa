...
class UnifiedPDFAnalyzer:
    ...
    # ★★★ v2.0 고속 설정 (인식 품질 유지하면서 속도 극대화) ★★★
    DPI = 130              # 180 → 130 (MAX_IMAGE_PX=1200 타겟에 최적)
    MAX_PAGES = 50
    MAX_IMAGE_PX = 1200    # API 전송용 최대 크기
    RPM_DELAY = 0.0        # 제거 (Rate Limiter에 위임) (0.3 → 0)
    RPM_DELAY_JITTER = 0.0 # 지터 제거
    UNKNOWN_BATCH_SIZE = 8 # 배치 크기 확대 (6 → 8)
    MAX_WORKERS = 5        # 워커 수 확대 (4 → 5)
    
    # ★ v2.0: 카운터 전용 락 (API 호출 직렬화 제거)
    _api_call_counter_lock = threading.Lock()
    ...
    def analyze(
        ...
        # ★ v2.0: 5단계 제거 — analyze_pdf_unified() 래퍼에서 조건부로 처리
        # (이전: 여기서도 전용 추출기 호출 → 중복 API 호출 발생)
    ...
    def _extract_pages(self, pdf_path: str) -> List[Tuple[Image.Image, str]]:
        """★ v2.0 PDF 페이지 추출 (고속 이미지 파이프라인)
        
        최적화:
        1. pixmap.samples → PIL.frombytes (PNG 인코딩 건너뜀, 2-3배 빠름)
        2. 목표 크기에 맞는 최적 DPI 계산 (불필요한 고해상도 방지)
        3. 전처리 경량화 (대비+선명도만)
        """
        if not HAS_PYMUPDF:
        ...
        # 페이지 데이터 추출 (fitz는 스레드 안전하지 않음 → 메인 스레드에서)
        page_data = []
        for page_num in range(total_pages):
            page = doc.load_page(page_num)
            text = page.get_text("text")
            
            # ★ v2.0: 목표 px에 맞는 최적 DPI 계산
            page_rect = page.rect
            long_side_pt = max(page_rect.width, page_rect.height)
            optimal_dpi = int(self.MAX_IMAGE_PX * 72 / long_side_pt)
            dpi = max(90, min(optimal_dpi, self.DPI))
    ...
    def _preprocess_image(self, image: Image.Image) -> Image.Image:
        """★ v2.0 경량 이미지 전처리 — 대비+선명도만 (색상 정보 보존)"""
        if image.mode != 'RGB':
            image = image.convert('RGB')
        
        # 대비 + 선명도 강화 (밝기 조정 제거 — AI Vision은 자동 보정)
        try:
    ...
    def _throttle_before_request(self) -> None:
        """★ v2.0: Rate Limiter에 위임 — 여기서는 no-op"""
        pass

    def _generate_content_with_retry(self, *args, **kwargs):
        """Vision API 호출 (Gemini/Claude) + 429 시 지수 백오프·지터 재시도
        
        ★ v2.0: _api_lock 제거 → 진정한 병렬 호출. Rate Limiter가 동시성 관리.
        """
...
★ 모든 호수의 전용면적을 빠짐없이 개별 추출하세요.
- 같은 면적이라도 호마다 별도 항목으로 출력
- 전유부가 여러 페이지에 걸쳐있으면 모든 페이지의 호를 합산
- 전용면적은 숫자만 (㎡ 단위)
- 호수가 "제101호" 형식이면 "101"로 정리
...
JSON만 출력하세요."""
    ...
    def _apply_building_ledger_title(self, result: PublicHousingReviewResult, data: Dict):
        ...
        if data.get("basement_floors") is not None:
            try:
                bf = int(data["basement_floors"])
            except (ValueError, TypeError):
                bf = 0
            result.building_ledger_title.basement_floors = bf
            result.building_ledger_title.has_basement = bf > 0
        else:
        ...
        if data.get("elevator_count"):
            try:
                ec = int(data["elevator_count"])
            except (ValueError, TypeError):
                ec = 0
            result.building_ledger_title.elevator_count = ec
            result.building_ledger_title.has_elevator = ec > 0
    ...
    def _apply_building_ledger_exclusive(self, result: PublicHousingReviewResult, data: Dict):
        ...
        units = data.get("units", [])
        print(f"    [전유부] API 반환 호수: {len(units)}개")
        if units:
            ...
            for u in units:
                ...
                print(f"    [전유부] {unit_num}호 = {area}㎡")
                result.building_ledger_exclusive.units.append(ExclusiveUnit(unit_number=unit_num, exclusive_area=area))
            areas = [u.exclusive_area for u in result.building_ledger_exclusive.units if u.exclusive_area]
            if areas:
                print(f"    [전유부] 최소={min(areas)}㎡, 최대={max(areas)}㎡ ({len(areas)}개 호)")
...
def analyze_pdf_unified(
    ...
    single_shot: bool = True,     # ★★★ v3.0: SingleShot 모드 (기본 활성화)
) -> Tuple[PublicHousingReviewResult, Dict]:
    """
    통합 PDF 분석 — v3.0 SingleShot 최적화
    
    ★★★ v3.0 변경점 ★★★
    기존: 문서유형 판별 N회 + 상세추출 N회 = 10~20회 API → 429 폭탄
    변경: 전체 PDF를 1회 API 호출로 통합 분석 → 429 거의 발생 안 함
    
    분석기 우선순위:
      1. SingleShot (1회 API 호출, 기본) ← NEW
      2. PrecisionAnalyzer (10~15회 API, 폴백)
      3. UltraUnified (레거시 폴백)
    
    Args:
        pdf_path: PDF 파일 경로
        announcement_date: 공고일 (YYYY-MM-DD)
        provider: 'gemini'(권장) | 'claude'
        model_name: 미지정 시 자동
        precision_mode: True면 고정밀 분석기(폴백용)
        fast_mode: True면 초고속 병렬 분석기
        single_shot: True면 1회 API 호출 모드 (기본, 최우선)
    ...
    # ★★★ v3.0: SingleShot 모드 — 1회 API 호출로 전체 분석 (최우선) ★★★
    if single_shot:
        try:
            from core.single_shot_analyzer import SingleShotPDFAnalyzer
            analyzer = SingleShotPDFAnalyzer(provider=provider, model_name=model_name)
            result, meta = analyzer.analyze(pdf_path, announcement_date)
            print(f"\n[SingleShot] ★ API 1회 호출로 분석 완료!")
        except ImportError as ie:
            print(f"\n[SingleShot] 모듈 없음, 폴백: {ie}")
        except Exception as e:
            print(f"\n[SingleShot] 실패, 기존 분석기로 폴백: {e}")
            import traceback
            traceback.print_exc()
            result = None
    
    # 폴백 1: 초고속 모드 (이중검증)
    if result is None and fast_mode:
        try:
    ...
    # ★★★ 조건부 소유자 전용 추출기 (SingleShot에서 이미 포함, 대부분 SKIP) ★★★
    if result is not None:
        owner = result.housing_sale_application.owner_info
        owner_name_exists = bool(owner.name and str(owner.name).strip())
        
        if owner_name_exists:
            print(f"\n[소유자 추출] ★ 이미 추출됨 (이름: {owner.name}) → 전용 추출기 SKIP")
        else:
            print("\n[소유자 전용 추출기] 소유자 정보 부족 → 추출기 호출")
            try:
                ...
                print(f"  소유자 정보 {new_filled}/5 채워짐")
            except Exception as e:
                print(f"  [전용 추출기 오류] {e}")
    
    _elapsed = _time.time() - _t0
    print(f"\n★★★ [analyze_pdf_unified v3.0] 전체 소요: {_elapsed:.1f}초 ★★★\n")
