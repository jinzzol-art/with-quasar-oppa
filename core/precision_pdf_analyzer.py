...
class PrecisionPDFAnalyzer:
    ...
    # 적응형 DPI 설정 (★v2.0 속도 최적화)
    DEFAULT_DPI = 120  # 기본 DPI (150 → 120, API가 이미지 인식하기 충분)
    HIGH_DPI = 150     # 고해상도 (작은 문서용) (200 → 150)
    LOW_DPI = 90       # 저해상도 (대용량 문서용) (100 → 90)
    
    MAX_PAGES = 50
    MAX_IMAGE_PX = 1200  # API 전송용 최대 크기
    MAX_SAFE_PIXELS = 100_000_000  # 1억 픽셀 이상은 저해상도로 처리
    
    # ★ API 설정 (v2.0 고속화 - Rate Limiter가 간격을 관리하므로 여기서는 최소화)
    MIN_RPM_DELAY = 0.0   # 제거 (Rate Limiter에 위임) (0.1 → 0)
    MAX_RPM_DELAY = 0.05  # 최소 지터만 (0.3 → 0.05)
    MAX_RETRIES = 5
    RETRY_BASE_DELAY = 5
    
    # ★ 병렬 처리 설정 (v2.0 진정한 병렬)
    MAX_WORKERS = 5   # 동시 API 호출 수 (4 → 5, Rate Limiter와 동기화)
    _api_call_counter_lock = threading.Lock()  # 카운터 전용 락 (호출 직렬화 아님)
    ...
    def _extract_pages_high_quality(self, pdf_path: str) -> List[Tuple[Image.Image, str, Image.Image]]:
        ...
        최적화:
        1. 목표 픽셀 크기(MAX_IMAGE_PX)에 맞는 최적 DPI를 직접 계산
           → 불필요한 고해상도 추출 후 축소 과정 제거
        2. pixmap → PIL 변환 시 tobytes("ppm") 사용 (PNG 인코딩 skip)
        3. 전처리 최소화 (대비+선명도만, 밝기/노이즈 제거 생략)
        ...
        for page_num in range(min(len(doc), self.MAX_PAGES)):
            ...
            # ★ v2.0: 목표 픽셀 크기에 맞는 최적 DPI 직접 계산
            page_rect = page.rect
            page_width_pt = page_rect.width
            page_height_pt = page_rect.height
            long_side_pt = max(page_width_pt, page_height_pt)
            
            # 목표: long_side가 MAX_IMAGE_PX가 되는 DPI 계산
            # pixels = pt * dpi / 72 → dpi = pixels * 72 / pt
            optimal_dpi = int(self.MAX_IMAGE_PX * 72 / long_side_pt)
            # DPI 범위 제한 (최소 90, 최대 200)
            dpi = max(self.LOW_DPI, min(optimal_dpi, self.HIGH_DPI))
            
            # 안전 픽셀 수 검증
            est_px = int(page_width_pt * dpi / 72) * int(page_height_pt * dpi / 72)
            if est_px > self.MAX_SAFE_PIXELS:
                dpi = max(72, int((self.MAX_SAFE_PIXELS * 0.8 * 72 * 72 / (page_width_pt * page_height_pt)) ** 0.5))
            ...
            try:
                ...
                # ★ v2.0: PPM/raw 방식으로 PIL 변환 (PNG 인코딩 건너뜀)
                try:
                    # samples 직접 접근 (가장 빠름)
                ...
                # API 전송용 리사이즈 (이미 최적 DPI로 추출했으므로 대부분 불필요)
    ...
    def _preprocess_for_ocr(self, image: Image.Image) -> Image.Image:
        """★ v2.0 경량 OCR 전처리 — 대비+선명도만 (색상·인감 정보 보존)
        
        OpenCV 적응형 이진화를 제거:
        - 이진화는 빨간 인감/도장 색상 정보를 파괴
        - Gemini/Claude Vision은 컬러 이미지에서 더 정확한 인식
        """
        # ★ copy() 생략 — enhance()가 이미 새 이미지 반환
        try:
    ...
    def _identify_all_pages(self, pages: List[Tuple]) -> List[Tuple[DocType, float]]:
        """★ v2.0 문서 유형 판별 — 미확인 페이지 배치 처리로 API 호출 최소화"""
        results: List[Tuple[DocType, float]] = [(DocType.UNKNOWN, 0.0)] * len(pages)
        unknown_indices: List[int] = []
        
        # 1차: 텍스트 기반 감지 (API 호출 없음)
        for page_num, (original, text, processed) in enumerate(pages):
            text_type, text_conf = self._detect_by_text(text)
            text_len = len((text or "").replace(" ", "").replace("\n", ""))
            
            if text_len >= 50 and text_conf >= 0.7:
                self.log(f"    페이지 {page_num + 1}: {text_type.value} (텍스트, 신뢰도: {text_conf:.0%})")
                results[page_num] = (text_type, text_conf)
            else:
                self.log(f"    페이지 {page_num + 1}: 텍스트 {text_len}자 → AI 배치 판별 대기")
                unknown_indices.append(page_num)
        
        # 2차: 미확인 페이지 배치 처리 (1회 API 호출로 최대 10개 판별)
        if unknown_indices:
            self.log(f"    ★ 미확인 {len(unknown_indices)}페이지 배치 AI 유형 판별...")
            batch_size = 8  # 한 번에 최대 8개 이미지
            for batch_start in range(0, len(unknown_indices), batch_size):
                ...
                batch_images = [pages[i][0] for i in batch_indices]
                
                prompt = f"""다음 {len(batch_images)}개 이미지를 순서대로 보고, 각 이미지의 문서 유형을 판별하세요.
...
출력 (JSON 배열만):
[{{"document_type": "문서유형명", "confidence": 0.9}}, ...]"""
                
                try:
                    response = self._call_api(prompt, batch_images)
                    data = self._parse_json(response)
                    if isinstance(data, list):
                        for j, item in enumerate(data[:len(batch_indices)]):
                            if isinstance(item, dict):
                                type_str = item.get("document_type", "")
                                conf = float(item.get("confidence", 0.7))
                                doc_type = self._map_type_string(type_str)
                                idx = batch_indices[j]
                                results[idx] = (doc_type, conf)
                                self.log(f"    페이지 {idx + 1}: {doc_type.value} (AI 배치, 신뢰도: {conf:.0%})")
                    elif isinstance(data, dict):
                        # 단일 객체 응답 처리
                        type_str = data.get("document_type", "")
                        conf = float(data.get("confidence", 0.7))
                        doc_type = self._map_type_string(type_str)
                        results[batch_indices[0]] = (doc_type, conf)
                except Exception as e:
                    self.log(f"      [오류] 배치 유형 판별 실패: {e}")
...
JSON만 출력하세요."""
    ...
    def _call_api(self, prompt: str, images: List[Image.Image]) -> str:
        """API 호출 (재시도 포함, 진정한 병렬 - Rate Limiter가 동시성 관리)"""
        # 카운터만 원자적으로 증가 (호출을 직렬화하지 않음)
        with self._api_call_counter_lock:
            self._api_calls += 1
